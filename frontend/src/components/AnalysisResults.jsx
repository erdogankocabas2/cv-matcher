...
export default function AnalysisResults({ results, onOpenAuthModal }) {
  ...
  const renderLockedState = (title) => (
    <div className="bg-slate-50/80 p-8 rounded-3xl border border-slate-200/60 border-dashed text-center flex flex-col items-center justify-center min-h-[280px] animate-fadeIn">
      <div className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl mb-4 shadow-inner">
        <Lock className="w-6 h-6 text-indigo-600 animate-bounce" />
      </div>
      ...
      <button
        ...
        className="mt-5 py-3 px-6 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl text-xs font-black shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-95 transition-all"
      >
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 bg-slate-100 p-1 rounded-2xl w-full border border-slate-200/40">
        <button
          ...
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold transition-all ${subTab === 'general' ? 'bg-white text-indigo-650 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
        >
          Genel Rapor
        </button>
        <button
          onClick={() => setSubTab('suggestions')}
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold transition-all ${subTab === 'suggestions' ? 'bg-white text-indigo-650 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
        >
          Tavsiyeler & Sorular
        </button>
        <button
          onClick={() => setSubTab('ats')}
          className={`flex-1 py-2.5 rounded-xl text-xs font-extrabold transition-all flex items-center justify-center gap-1.5 ${subTab === 'ats' ? 'bg-white text-indigo-650 shadow-sm' : 'text-slate-500 hover:text-slate-800'}`}
        >
          ...
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-md grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
            <div className="flex flex-col items-center justify-center md:border-r md:border-slate-100 pr-0 md:pr-6 shrink-0">
              <div className="relative flex items-center justify-center w-24 h-24">
                ...
                <span className="absolute text-2xl font-extrabold text-slate-800">%{uygunluk_skoru}</span>
              </div>
              <span className="mt-2 text-xs font-bold text-slate-400 uppercase tracking-wider">Uygunluk Skoru</span>
            </div>

            <div className="md:col-span-3">
              <h3 className="text-lg font-bold text-slate-800 mb-2">Genel Değerlendirme</h3>
              <p className="text-sm text-slate-650 leading-relaxed font-medium">{ozet}</p>
            </div>
          </div>
              ...
              {guclu_yonler && guclu_yonler.length > 0 ? (
                <ul className="space-y-3">
                  {guclu_yonler.map((item, idx) => (
                    <li key={idx} className="flex gap-2 text-sm text-slate-600 items-start font-medium">
              ...
              optimizasyon_onerileri && optimizasyon_onerileri.length > 0 ? (
                <ul className="space-y-3">
                  {optimizasyon_onerileri.map((item, idx) => (
                    <li key={idx} className="flex gap-2.5 text-sm text-slate-600 items-start font-medium">
      ...
      {/* --- SEKME 3: ATS & CV İYİLEŞTİRME REHBERİ (YENİ) --- */}
      {subTab === 'ats' && (
        <div className="space-y-6 animate-fadeIn">
          {isGuideLocked() ? (
            ...
            <>
              ...
              <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-md">
                <h3 className="text-md font-bold text-slate-800 mb-2 flex items-center gap-2 border-b border-slate-50 pb-2">
                  <Sparkles className="w-5 h-5 text-indigo-600" />
                  Eksik ATS Anahtar Kelimeleri
                </h3>
                <p className="text-xs text-slate-500 mb-4">Aşağıdaki kelimeler ilanda kritik olup CV'nizde eksiktir. Bunları CV'nizin ilgili yerlerine ekledikçe işaretleyebilirsiniz:</p>
                    ...
                    {eksik_ats_anahtar_kelimeleri.map((kw, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`py-2 px-4 rounded-xl text-xs font-bold border transition-all flex items-center gap-2 ${
                            isChecked
                        ...
                        >
                          <span className={`w-3.5 h-3.5 rounded-md flex items-center justify-center border transition-all text-white ${
                            isChecked ? 'bg-emerald-500 border-emerald-600' : 'bg-white border-slate-300'
                          }`}>
                ...
                ) : <p className="text-sm text-slate-400">İlanla eşleşen tüm kritik anahtar kelimeler CV'nizde yer alıyor!</p>}
              </div>

              {/* Satır Satır İyileştirme Kılavuzu */}
              <div className="space-y-4">
                <h3 className="text-md font-extrabold text-slate-800 flex items-center gap-2 px-1">
                ...
                {cv_optimizasyon_kilavuzu && cv_optimizasyon_kilavuzu.length > 0 ? (
                  cv_optimizasyon_kilavuzu.map((item, idx) => (
                    <div key={idx} className="bg-white rounded-3xl border border-slate-100 shadow-md overflow-hidden">
                      <div className="p-5 space-y-4">
                        {/* Before / After Columns */}
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                          {/* Before */}
                          <div className="md:col-span-5 bg-rose-50/40 p-4 rounded-2xl border border-rose-100/50">
                            <span className="inline-block px-2.5 py-1 bg-rose-100 text-rose-700 rounded-md text-[10px] font-black uppercase mb-2">CV'deki Mevcut Satır</span>
                            <p className="text-xs font-semibold text-slate-600 italic leading-relaxed">"{item.mevcut_cumle}"</p>
                          </div>
                          ...
                          {/* After */}
                          <div className="md:col-span-5 bg-emerald-50/40 p-4 rounded-2xl border border-emerald-100/50 relative">
                            <span className="inline-block px-2.5 py-1 bg-emerald-100 text-emerald-700 rounded-md text-[10px] font-black uppercase mb-2">ATS Uyumlu Önerilen Satır</span>
                            <p className="text-xs font-bold text-slate-800 leading-relaxed">"{item.onerilen_cumle}"</p>
                          </div>
                        </div>

                        {/* Explanation */}
                        <div className="bg-indigo-50/20 p-4 rounded-2xl border border-indigo-100/50 flex gap-2.5 items-start">
                          <div className="p-1 bg-indigo-100 rounded text-indigo-650 mt-0.5 shrink-0">
                            <Sparkles className="w-3.5 h-3.5" />
                          </div>
                          <div>
                            <span className="block text-[10px] font-black text-indigo-700 uppercase">İK Danışmanı Notu:</span>
                            <p className="text-xs text-slate-650 font-medium mt-0.5 leading-relaxed">{item.aciklama}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ))
                ) : <p className="text-sm text-slate-400">Yazım önerisi bulunamadı.</p>}
              </div>
            </>
          )}
        </div>
